...
@implementation CybsAGGcmMathComponents
           ...
           hashSubkey:(IAGBlockType)hashSubkey
{
    ...
    {
        IAGBlockType xor;
        [CybsAGBitwiseComponents getXorBlock:xor withBlock:y andBlock:(buffer + i)];

        [CybsAGGcmMathComponents getProductBlock:y byMultiplyingBlock:xor andBlock:hashSubkey];
    }
                    ...
                    error:(NSError **)error
{
    ...
    for (IAGSizeType i = 0; i < (bufferSize - uncompleteBlockSize); i += sizeof(IAGBlockType))
    {
        ...
        BOOL success = [CybsAGAesComponents getCipheredBlock:cipheredBlock
    ...
    if (uncompleteBlockSize > 0)
    {
        ...
        [CybsAGBitwiseComponents getMostSignificantBytes:msb
                                             withSize:uncompleteBlockSize
                                             inBuffer:cipheredBlock
                                             withSize:sizeof(IAGBlockType)];

        [CybsAGBitwiseComponents getXorBuffer:(y + bufferSize - uncompleteBlockSize)
                             ...
                             size:(IAGSizeType)size
{
    ...
    IAGUInt32Type lsb;
    [CybsAGBitwiseComponents getLeastSignificantBytes:(IAGUCharType *)&lsb
    ...
    // still correct
    lsb = [CybsAGGcmEndianness swapUInt32HostToGcm:([CybsAGGcmEndianness swapUInt32GcmToHost:lsb] + 1)];

    if (size != sizeof(IAGUInt32Type))
    {
        [CybsAGBitwiseComponents getMostSignificantBytes:incBuffer
               ...
               andBlock:(IAGBlockType)y
{
    ...
    for (IAGUInt8Type i = 0; i <= IAGMaxBitPositionInABlock; i++)
    {
        ...
        if ([CybsAGBitwiseComponents isMostSignificantBitActivatedAtPosition:i inBlock:x])
        {
            IAGBlockType zi;
            memcpy(zi, z, sizeof(IAGBlockType));

            [CybsAGBitwiseComponents getXorBlock:z withBlock:zi andBlock:v];
        }
        ...
        [CybsAGBitwiseComponents getSingleRightShiftedBlock:v withBlock:vi];

        if ([CybsAGBitwiseComponents isLeastSignificantBitActivatedAtPosition:0 inBlock:vi])
        {
            memcpy(vi, v, sizeof(IAGBlockType));

            [CybsAGBitwiseComponents getXorBlock:v withBlock:vi andBlock:r];
        }
    }
