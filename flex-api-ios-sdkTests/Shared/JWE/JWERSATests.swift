...
class JWERSATests: RSACryptoTestCase {
    ...
    func testJWERoundtrip() {
        ...
        for algorithm in keyManagementModeAlgorithms {
            let header = JWEHeader(keyManagementAlgorithm: algorithm, contentEncryptionAlgorithm: .A256GCM)
            let payload = Payload(message.data(using: .utf8)!)
            let encrypter = Encrypter(keyManagementAlgorithm: algorithm, contentEncryptionAlgorithm: .A256GCM, encryptionKey: publicKeyAlice2048)!
            let jweEnc = try! JWE(header: header, payload: payload, encrypter: encrypter)
            XCTAssertNotNil(jweEnc)
        }
    }
    ...
    func testJWEEncryptionWithMismatchingHeaderAlg() {
        ...
        let payload = Payload(message.data(using: .utf8)!)
        let encrypter = Encrypter(keyManagementAlgorithm: .RSAOAEP, contentEncryptionAlgorithm: .A256GCM, encryptionKey: publicKeyAlice2048)!
    ...
    func testJWERoundtripWithNonRequiredJWEHeaderParameter() {
        for algorithm in keyManagementModeAlgorithms {
            var header = JWEHeader(keyManagementAlgorithm: algorithm, contentEncryptionAlgorithm: .A256GCM)
            header.kid = "kid"
